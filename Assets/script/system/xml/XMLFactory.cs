...
//            person.itemList.Add(loadItem(item.InnerText));
    ...
    public static List<Ability> loadSkillSet(string link) {
        ...
        foreach (XmlNode skill in xmlAbility) {
            ...
            ability.position = laodPosition(skill[XMLGame.XML_PERSON_POSITION]);
    ...
    public static Vector2 laodPosition(XmlNode xmlPosition) {
        var position = new Vector2 {
            x = int.Parse(xmlPosition[XMLGame.XML_PERSON_POSITION_X].InnerText),
            y = int.Parse(xmlPosition[XMLGame.XML_PERSON_POSITION_Y].InnerText)
        };
        return position;
    }
}